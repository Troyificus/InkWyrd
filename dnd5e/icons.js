...
// Marks for Trait / Action / Bonus Action / Reaction / Legendary Action.
const FEATURE_ICONS = {
  trait:      '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7" fill="none" stroke="currentColor" stroke-width="2.5"/></svg>',
  action:     '<svg viewBox="0 0 24 24"><path d="M12 3 L21 19 L3 19 Z" fill="currentColor"/></svg>',
  'bonus action': '<svg viewBox="0 0 24 24"><path d="M12 3 L21 19 L3 19 Z" fill="none" stroke="currentColor" stroke-width="2.5"/></svg>',
  reaction:   '<svg viewBox="0 0 24 24"><path d="M12 3 L21 12 L12 21 L3 12 Z" fill="currentColor"/></svg>',
  'legendary action': '<svg viewBox="0 0 24 24"><path d="M12 2 L14.5 9.5 L22 12 L14.5 14.5 L12 22 L9.5 14.5 L2 12 L9.5 9.5 Z" fill="currentColor"/></svg>'
};

function getFeatureIcon(category) {
  return FEATURE_ICONS[(category || '').toLowerCase()] || FEATURE_ICONS.trait;
}
